import numpy as np
from CEC17 import sum_diff_pow_func, zakharov_func, levy_func, modif_schwefel_func, happycat_func, hgbat_func
...
def cec13(pop, func_index):
    ...
    if func_index == 3 or func_index == 7 or func_index == 8 or func_index == 9 or func_index == 11 or func_index == 12\
        ...
        rotate = rotate[0: 2 * pop_dim * pop_dim].reshape(2, pop_dim, pop_dim)  # [2DD,1]->[2,D,D],rotatei=[2,:,:]
        ...
        rotate = rotate[0: pop_dim * pop_dim].reshape(pop_dim, pop_dim)  # [DD,1]->[D,D],rotatei=[:,:]
    ...
    if func_index == 1:
        ...
        fitness = sum_diff_pow_func(pop, shift=0., rotate=0., s_flag=0, r_flag=0)
        label = 'b1'
        # print(label)
    elif func_index == 32:
        fitness = zakharov_func(pop, shift=0., rotate=0., s_flag=0, r_flag=0)
        label = 'b2'
        # print(label)
    elif func_index == 33:
        fitness = levy_func(pop, shift=0., rotate=0., s_flag=0, r_flag=0)
        label = 'b3'
        # print(label)
    elif func_index == 34:
        fitness = modif_schwefel_func(pop, shift=0., rotate=0., s_flag=0, r_flag=0)
        label = 'b4'
        # print(label)
    elif func_index == 35:
        fitness = happycat_func(pop, shift=0., rotate=0., s_flag=0, r_flag=0)
        label = 'b5'
        # print(label)
    elif func_index == 36:
        fitness = hgbat_func(pop, shift=0., rotate=0., s_flag=0, r_flag=0)
        label = 'b6'
        # print(label)
...
def bent_cigar_func(pop, shift, rotate, r_flag): #rotate[2, D, D]
    beta = 0.5
    pop_dim = pop.shape[0]
    pop_size = pop.shape[1]
    pop_s = shift_pop(pop, shift)
...
def step_rastrigin_func(pop, shift, rotate, r_flag):
    ...
    pop_sr = asy_pop_func2(pop_sr1, pop_sr, beta) #此处输出pop_sr=z，pop_sr1=y
    if r_flag == 1:
...
def grie_rosen_func(pop, shift, rotate, r_flag):
    ...
    if r_flag == 1:
        ...
        pop_sr = pop_s
    pop_sr = pop_s + 1 #原demo错了，按照公式应该是pop_sr+1
    for j in range(pop_size):
        f = 0.0
        for i in range(pop_dim-1):
            tmp1 = pop_sr[i, j]*pop_sr[i, j]-pop_sr[i+1, j]
